  ...
  if (size < 0) {
    ...
    for (int m = 0; m < size; m++) {
      ...
      for (int n = 1; n < size; n++) {
        X[m] += x[n] * (cos(2.0 * M_PI * m * n / size) -
                        I * sin(2.0 * M_PI * m * n / size));
      }
    }
  }
  return A;
...
void Hamming(int size, int left, int right, double para, double x[]) {
  ...
  for (int i = 0; i < size; i++) {
    if (i < right && i >= left) {
      x[i] =
          10 * x[i] *
          ((1 - para) - para * cos(2 * M_PI * (i - left) / (right - left - 1)));
    } else {
      x[i] = 0;
    }
  }
...
void Blackman(int size, int left, int right, double x[]) {
  ...
  for (int i = 0; i < size; i++) {
    if (i < right && i >= left) {
      x[i] = 10 * x[i] *
             (0.42 + 0.08 * cos(4 * M_PI * (i - left) / (right - left - 1)) -
              0.5 * cos(2 * M_PI * (i - left) / (right - left - 1)));
    } else {
      x[i] = 0;
    }
  }
